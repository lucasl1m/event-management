...
export function EventDetailClient({ eventId }: EventDetailClientProps) {
  ...
  if (isError || !eventQuery.data) {
    return (
      <ErrorState
        ...
        description="Verifique se o evento existe e tente novamente."
        onRetry={() => {
          eventQuery.refetch();
  ...
  return (
    <div className="flex flex-col gap-8 animate-[fadeIn_0.3s_ease_forwards] opacity-0">
      {/* Breadcrumb */}
      <nav aria-label="Navegação de caminho" className="flex items-center gap-1.5 text-sm">
        <Link
          href="/events"
        ...
        >
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-xs text-amber-400/80">
              Entradas e saídas de participantes não são mais permitidas.
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border/50 bg-card p-5">
          <h3 className="text-sm font-semibold text-foreground">Evolução de Entradas</h3>
        ...
        <div className="flex flex-col items-center gap-3 rounded-xl border border-border/50 bg-card p-5">
          <h3 className="w-full text-sm font-semibold text-foreground">Sucesso vs. Erro</h3>
      ...
      <section aria-labelledby="participants-heading">
        ...
        {participants.length === 0 ? (
          <EmptyState
            ...
            description="Quando houver participantes, eles aparecerão aqui."
          />
        ) : (
